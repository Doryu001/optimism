...
func TestEngineQueue_Finalize(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		fi := NewFinalizer(context.Background(), logger, &rollup.Config{}, &FinalizerConfig{}, l1F, ec)
...
func TestFinalizerConfig(t *testing.T) {
	...
	t.Run("uses defaults when config fields are nil", func(t *testing.T) {
		...
		require.Equal(t, uint64(defaultFinalityLookback), fi.finalityLookback, "should use default finality lookback when field is nil")
		require.Equal(t, uint64(finalityDelay), fi.finalityDelay, "should use default finality delay when field is nil")
	})
